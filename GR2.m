...
function [g11 g12 g13 g21 g22 g23 g31 g32 g33]=GR2(h,zx1,zy1,zz1,zx2,zy2,zz2)

ps=0.25;%泊松比
xs=sqrt((1-2*ps)/(2*(1-ps)));%S波速/P波速
mu=1;%剪切模量
q=h;%P波波数
k=q/xs;%S波波数
...
% g=(k*r*A1+B1+C1/(k*r)+D1/(k*r)^2)*exp(-i*k*r)+(k*r*A2+B2+C2/(k*r)+D2/(k*r)^2)*exp(-i*k*r);
% g=(k*r*A1+B1+C1/(k*r)+D1/(k*r)^2)*exp(-i*k*r)+(k*r*A2+B2+C2/(k*r)+D2/(k*r)^2)*exp(-i*k*r);%cuo cuo cuo 
...
for ii=1:3
 ...
 GRT(ii,jj)=(f2*dt(ii,jj)+(f1-f2)*gm(ii)*gm(jj))/(4*pi*mu*r);
